...
parser.add_argument('--model', '-m', metavar='MODEL', default='efficientdet_d1',
...
def validate(args):
    ...
    # )
    bench = pt.detection_models.__dict__[args.model](match_tf_same_padding=True)
    input_size = bench.pretrained_settings["input_size"] # bench.config.image_size
    ...
    with torch.no_grad():
        for i, (input, target) in enumerate(loader):
            # print(input.dtype)
            output = bench.predict(input) #, target['img_scale'], target['img_size'])
            output = output.cpu()
            sample_ids = target['img_id'].cpu()
            for index, sample in enumerate(output):
                image_id = int(sample_ids[index])
                for det, scale in zip(sample, target['img_scale']):
                    score = float(det[4])
                    if score < .001:  # stop when below this threshold, scores in descending order
                        break
                    coco_det = dict(
                        image_id=image_id,
                        bbox=(det[0:4] * scale).tolist(),
                        score=score,
